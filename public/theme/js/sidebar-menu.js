...
if ($('#pageWrapper').hasClass('compact-wrapper')) {
    ...
    // use pathname comparison to avoid '/' matching every URL
    var current = window.location.pathname;
    $(".sidebar-wrapper nav ul>li a").filter(function () {

        var link = $(this).attr("href");
        // skip empty or anchor-only links (e.g. '#' or '') which resolve to '/'
...
if ($('#pageWrapper').hasClass('compact-wrapper')) {
    ...
    // exact match using pathname instead of href substring
    var current = window.location.pathname;
    $(".sidebar-wrapper nav #sidebar-menu ul .simplebar-mask li a").filter(function () {

        var link = $(this).attr("href");
        // ignore anchors or empty strings which map to home path
        if (link && link !== '#' && link.trim() !== '') {
            var linkPath;
            try {
                linkPath = new URL(link, window.location.origin).pathname;
